...
def oversub(D, p, allocation, choreographers, choreo_min, choreo_max):
...
def undersub(D, p, allocation, choreographers, choreo_min, choreo_max):
    done = False
    while not done:
        done = True
        demand = np.sum(allocation, axis=0)
...
def final_allocation(D, p, allocation, choreographers, choreo_min, choreo_max):
    p = oversub(D, p, allocation, choreographers, choreo_min, choreo_max)
    allocation = D.allocation(p, choreographers)
    return p, undersub(D, p, allocation, choreographers, choreo_min, choreo_max)